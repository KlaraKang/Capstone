...
d3.csv('./Dataset/FundingPerSchool.csv', (d, i, columns) => {  
  ...
  xScale = d3.scalePoint()
          //  .domain(["2018-19","2019-20","2020-21","2021-22"])
          .domain(FPdata.map(d => d.AY))
       //   .domain(d3.map(FPdata, d=>d.AY))
          .range([margin.left, innerWidth/2-margin.right])

  let yScale1 = d3.scaleLinear()
          .domain([d3.min(boroughFP, d=>d3.min(d.values, i=>i.fund))/1000, 
                   d3.max(boroughFP, d=> d3.max(d.values, i=>i.fund))/1000])
          .range([height - margin.top - margin.bottom, margin.bottom])

  let yScale2 = d3.scaleLinear()
          .domain([d3.min(boroughIM, d=>d3.min(d.values, i=>i.fund))/1000, 
                   d3.max(boroughIM, d=> d3.max(d.values, i=>i.fund))/1000])
          .range([height - margin.top - margin.bottom, margin.bottom]) 
          
  let yScale3 = d3.scaleLinear()
          .domain([d3.min(boroughPS, d=>d3.min(d.values, i=>i.fund))/1000, 
                   d3.max(boroughPS, d=> d3.max(d.values, i=>i.fund))/1000])
          .range([height - margin.top - margin.bottom, margin.bottom]) 
  ...
  let svg1 = container1.append("svg")
        .attr("width", innerWidth/2 + margin.right) //width/2 +margin.right)
        .attr("height", height) 
  ...
  svg1.append("text")
          ...
          .text("Average School Funding per Pupil")
...
/* DRAW CHART 2. SCHOOL FUNDING FOR INSTRUCTIONAL MEDIA */        
  ...
  svg2.append("text")
          ...
          .style("font-size", "14px")
          .style("font-weight", "bold")
          .text("Average School Funding for Instructional Media")
...
*/
  ...
  svg3.append("text")
          ...
          .style("font-size", "14px")
          .style("font-weight", "bold")
          .text("Average School Funding for Student Support Services");  
